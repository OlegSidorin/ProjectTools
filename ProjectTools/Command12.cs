...
namespace ProjectTools
{
    ...
    class Command12 : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            string matName = Interaction.InputBox("Содержится в названии материала, который нужно заменить:", "Замена материала пола", "Бетон");
            ...
            var result = System.Windows.Forms.MessageBox.Show($"Заменить эти материалы:\n{listOfMaterials}?", caption,
                                 System.Windows.Forms.MessageBoxButtons.YesNo,
                                 System.Windows.Forms.MessageBoxIcon.Question);
            if (result == System.Windows.Forms.DialogResult.Yes)
            {
                foreach (var element in floorTypes)
                {
                    try
                    {
                        FloorType wallType = element as FloorType;
                        ...
                        if (wallType != null)
                        {
                            ...
                            if (structure != null)
                            {
                                if (structure.LayerCount != 0)
                                {
                                    ...
                                    if (ls.Count != 0)
                                    {
                                        using (Transaction t = new Transaction(doc, "Floor Material Change"))
                                        {
                                            t.Start();
